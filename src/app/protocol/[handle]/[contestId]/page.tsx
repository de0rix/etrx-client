...
import {
  getUserContestProtocol,
  GetUserContestProtocolArgs,
...
function ProtocolClientPage() {
  ...
  const columns: Column<UserContestProtocolForTable>[] = useMemo(
    () => [
      {
        key: 'time',
        header: t('protocol:tableHeaders.time'),
        accessor: 'time',
        render: (item) => unixToFormattedDateTime(item.time, t),
      },
      {
        key: 'index',
        header: t('protocol:tableHeaders.index'),
        accessor: 'index',
      },
      {
        key: 'participantType',
        header: t('protocol:tableHeaders.participantType'),
        accessor: 'participantType',
      },
      {
        key: 'programmingLanguage',
        header: t('protocol:tableHeaders.programmingLanguage'),
        accessor: 'programmingLanguage',
      },
      {
        key: 'verdict',
        header: t('protocol:tableHeaders.verdict'),
        accessor: 'verdict',
      },
    ],
    [t],
  ...
  const tableData: UserContestProtocolForTable[] = submissions.map(
    (sub, index) => ({ ...sub, id: `${sub.index}-${index}` }),
  );
      ...
      <Table
        ...
        onRowClick={(sub) => {
          if (contest?.source?.toLowerCase() === 'ioi') {
            window.open(
              `https://ioi.contest.codeforces.com/group/32KGsXgiKA/contest/${contestId}/problem/${sub.index}`,
            );
          } else if (!contest?.gym) {
            window.open(
              `https://codeforces.com/problemset/problem/${contestId}/${sub.index}`,
            );
          } else {
            window.open(
              `https://codeforces.com/gym/${contestId}/problem/${sub.index}`,
            );
          }
        }}
      />
