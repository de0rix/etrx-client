...
function ProtocolClientPage() {
  ...
  const columns: Column<UsersProtocolForTable>[] = useMemo(
    () => [
      ...
      {
        key: 'userName',
        header: t('protocol:tableHeaders.userName'),
        accessor: 'userName',
      },
      {
        key: 'contestsCount',
        header: t('protocol:tableHeaders.contestsCount'),
  ...
  const tableData: UsersProtocolForTable[] = submissions.map((sub) => ({
    ...sub,
    id: sub.userName,
  }));
      ...
      <div className="m-auto rounded-md h-fit px-4 py-2 w-fit bg-background-shade">
        ...
        <div className="flex items-center">
          ...
          <DatePicker
            ...
            className="border-[1.5px] w-[150px] border-solid border-black rounded-[6px] px-2 py-1 bg-[--background] mr-4"
          />
          <label htmlFor="setContestId" className="mr-2">
            {t('protocol:filters.setContestId')}
          </label>
          <input
            id="setContestId"
            type="number"
