...
import {
  ...
  GetRanklistRowsArgs,
  updateRanklistRows,
} from '@/app/services/ranklistRows';
import { Column, SortOrder } from '@/app/models/TableTypes';
import {
  Problem,
  RanklistRow,
  RanklistRowForTable,
} from '@/app/models/Ranklist';
...
function ContestIdClientPage() {
  ...
  const participantType = useMemo(() => {
    ...
    return contest?.source?.toLowerCase() === 'ioi'
      ? 'PRACTICE'
      : DEFAULT_PARTICIPANT_TYPE;
  }, [searchParams, contest]);
  const sortField = useMemo(
    () =>
      (searchParams.get('sortField') as keyof RanklistRow) ||
      DEFAULT_SORT_FIELD,
    [searchParams],
  );
    ...
    const fetchContest = async () => {
      try {
        ...
        if (!contestResponse.ok)
          throw new Error(
            t('common:error', { statusCode: contestResponse.status }),
          );
  ...
  useEffect(() => {
    ...
    const fetchRanklist = async () => {
      ...
      try {
        const shouldUpdate =
          !contest.isContestLoaded || participantType !== 'CONTESTANT';
        ...
        if (!ranklistResponse.ok)
          throw new Error(
            t('common:error', { statusCode: ranklistResponse.status }),
          );
  ...
  const columns: Column<RanklistRowForTable>[] = useMemo(() => {
    const staticColumns: Column<RanklistRowForTable>[] = [
      {
        key: 'username',
        header: t('contestId:tableHeaders.userName'),
        accessor: 'username',
      },
      {
        key: 'city',
        header: t('contestId:tableHeaders.city'),
        accessor: 'city',
      },
      {
        key: 'organization',
        header: t('contestId:tableHeaders.organization'),
        accessor: 'organization',
      },
      {
        key: 'grade',
        header: t('contestId:tableHeaders.class'),
        accessor: 'grade',
      },
      {
        key: 'points',
        header: t('contestId:tableHeaders.points'),
        accessor: 'points',
      },
      {
        key: 'solvedCount',
        header: t('contestId:tableHeaders.solvedCount'),
    ...
    if (participantType === 'ALL') {
      staticColumns.push({
        key: 'participantType',
        header: t('contestId:tableHeaders.participantType'),
        accessor: 'participantType',
      });
    }

    const dynamicColumns: Column<RanklistRowForTable>[] = problems.map(
      (problem) => {
        const problemHost =
          contest?.source?.toLowerCase() === 'ioi'
        ...
        return {
          ...
          isSortable: false,
          render: (row) => {
            const result = row.problemResults.find(
              (pr) => pr.index === problem.index,
            );
            if (!result) return '';
            if (result.points > 0)
              return result.points === 1 ? '+' : result.points;
            if (result.rejectedAttemptCount > 0)
              return `-${result.rejectedAttemptCount}`;
            return '';
          },
        };
      },
  ...
  const participantFilterOptions: Option[] = [
    ...
    {
      label: t('contestId:filters.options.outOfCompetition'),
    ...
    <>
      ...
      {loadingTime !== null && (
        <p className="text-center">
          {t('common:loadingTime', {
            seconds: (loadingTime / 1000).toFixed(2),
          })}
        </p>
      )}
    </>
