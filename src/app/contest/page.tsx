...
function ContestClientPage() {
  ...
  const sortField = useMemo(
    () =>
      (searchParams.get('sortField') as keyof Contest) || DEFAULT_SORT_FIELD,
    [searchParams],
  );
  ...
  const handleSortChange = (newSortField: keyof ContestForTable) => {
    const effectiveSortField =
      newSortField === 'id' ? 'contestId' : newSortField;
    const newSortOrder =
  ...
  const tableData: ContestForTable[] = contests.map((c) => ({
    ...c,
    id: c.contestId,
  }));
