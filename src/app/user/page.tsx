...
function UserClientPage() {
  ...
  const sortField = useMemo(
    () => (searchParams.get('sortField') as keyof User) || DEFAULT_SORT_FIELD,
    [searchParams],
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    const args = new GetUsersArgs(null, null, sortField, sortOrder === 'asc');
  ...
  const handleAddUser = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setActionError(
        `Не удалось добавить пользователя: ${(err as Error).message}`,
      );
    } finally {
  ...
  const handleDeleteUser = useCallback(
    async (handle: string) => {
      ...
      try {
        ...
        setActionError(
          `Не удалось удалить пользователя: ${(err as Error).message}`,
        );
      }
    },
    [fetchData],
  ...
  const columns: Column<UserForTable>[] = useMemo(
    () => [
      {
        key: 'handle',
        header: t('user:tableHeaders.handle'),
        accessor: 'handle',
      },
      {
        key: 'firstName',
        header: t('user:tableHeaders.firstName'),
        accessor: 'firstName',
      },
      {
        key: 'lastName',
        header: t('user:tableHeaders.lastName'),
        accessor: 'lastName',
      },
      {
        key: 'organization',
        header: t('user:tableHeaders.organization'),
      ...
      { key: 'grade', header: t('user:tableHeaders.grade'), accessor: 'grade' },
      {
        ...
        render: (user) => (
          <div className="flex justify-center">
            <button
              type="button"
              className="rounded border border-red-500 px-2 py-1 text-red-500 hover:bg-red-500 hover:text-white"
              onClick={(event) => {
                event.stopPropagation();
                void handleDeleteUser(user.handle);
              }}
            >
  ...
  const tableData: UserForTable[] = users.map((user) => ({
    ...user,
    id: user.handle,
  }));
  ...
  return (
    <>
      <h1 className="text-3xl w-full text-center font-bold my-5">
        {t('user:usersTableTitle')}
      </h1>
      <div className="mb-5 flex flex-col items-center gap-3">
        <button
          type="button"
          className="rounded bg-[var(--main)] px-4 py-2 font-bold text-[var(--main-white)]"
          onClick={() => {
        ...
        {isAddMenuOpen && (
          <form
            ...
            className="grid w-full max-w-2xl grid-cols-1 gap-3 rounded-lg border border-[var(--background-shade1)] p-4 sm:grid-cols-2"
          >
            {(Object.keys(newUser) as Array<keyof User>).map((field) => (
              <label key={field} className="flex flex-col gap-1">
                ...
                <input
                  required
                  className="rounded border border-[var(--background-shade1)] bg-[var(--background)] px-3 py-2"
                  value={newUser[field]}
                  onChange={(event) => updateNewUser(field, event.target.value)}
                />
              </label>
            ))}
            <div className="flex gap-3 justify-center sm:col-span-2">
              ...
              <button
                type="button"
                disabled={isSubmitting}
                className="rounded border border-[var(--background-shade1)] px-4 py-2"
                onClick={() =>
                  setNewUser({
                    handle: '',
                    firstName: '',
      ...
      <Table
        ...
        onRowClick={(user) =>
          window.open(`https://codeforces.com/profile/${user.handle}`)
        }
      />
    </>
  );
